...
function defaultBranding(){
  return {
    ...
    sidebarTagline:'Dashboard Operativo',
    initial:'V',
    logoUrl:'',
    primary:'#FF6D00',
    secondary:'#00D4FF'
...
// Auto-login con parámetros de URL o credenciales
async function doAutoLogin(){
  const apiKey=document.getElementById('loginApiKey')?.value?.trim();
  const listId=document.getElementById('loginListId')?.value?.trim();
  const sessionId=document.getElementById('loginSessionId')?.value?.trim();
  
  if(!apiKey || !listId){
    alert('⚠️ Ingresa API Key y List ID de ClickUp');
    return;
  }
  
  try {
    showLoading('Validando conexión con ClickUp...');
    
    // Validar conexión con ClickUp
    ...
    const user={
      username:userData.user?.username || 'Usuario',
      name:userData.user?.username || 'Usuario',
      email:userData.user?.email || 'user@clickup.com',
      avatar:(userData.user?.username || 'U').charAt(0).toUpperCase(),
      color:'var(--primary)',
      apiKey:apiKey,
      listId:listId,
      sessionId:sessionId
    ...
    APP.token='session_'+sessionId;
    APP.currentUser=user;
    
    // Guardar en localStorage
    localStorage.setItem('vylex_sessionId',sessionId);
    localStorage.setItem('vylex_apiKey',apiKey);
    localStorage.setItem('vylex_listId',listId);
    ...
    // Cargar datos después de 1 segundo
...
async function doLogin(){
  ...
  if(!email || !pass) {
    toast('warning', '⚠️ Ingresa email y contraseña');
    return;
  }
  ...
  try {
    ...
    if(!response.ok){
      ...
      throw new Error(errData.error || 'Acceso denegado');
    }
    ...
    // Configurar ClickUp si el server devuelve las claves
    if(data.clickup) {
      CONFIG.API_KEY = data.clickup.apiKey || CONFIG.API_KEY;
      CONFIG.LIST_ID = data.clickup.listId || CONFIG.LIST_ID;
    }
    
    // Guardar sesión en localStorage para persistencia básica
    localStorage.setItem('vylex_token', APP.token);
    localStorage.setItem('vylex_user', JSON.stringify(APP.currentUser));
    ...
    setTimeout(()=>{
      scr.classList.add('hidden');
      document.getElementById('mainApp').classList.remove('hidden');
      setUserUI(APP.currentUser);
      
      // Intentar cargar datos pero sin bloquear si ClickUp falla (manejo interno en syncClickUp)
      loadSavedData();
      loadCustomKanbans();
      syncClickUp();
      hideLoading();
...
document.getElementById('newKanbanBtn')?.addEventListener('click', showCreateKanbanModal);
...
const SECTION_META={
  ...
  tableros:['Tableros & Proyectos','Gestión de vistas Kanban y proyectos manuales'],
...
const SECTION_META_I18N={
  ...
  en:{
    ...
    tableros:['Boards & Projects','Manage kanban views and manual projects'],
  ...
  pt:{
    ...
    tableros:['Quadros & Projetos','Gerencie visualizações Kanban e projetos manuais'],
...
const I18N={
  es:{
    'nav.main':'Principal','nav.dashboard':'Dashboard','nav.panoramic':'Vista Panorámica','nav.clients':'Clientes','nav.implementations':'Implementaciones','nav.active':'Activos','nav.canceled':'Cancelados','nav.alerts':'Alertas','nav.cs':'CS Dashboard','nav.opa':'OPA / Hola Suite','nav.projects':'Proyectos','nav.boards':'Tableros','nav.kanban':'Kanban','nav.channels':'Canales','nav.analysis':'Análisis','nav.consultants':'Consultores','nav.bi':'BI Dashboard','nav.sales':'Ventas & Leads','nav.goals':'Metas','nav.tools':'Herramientas','nav.improvements':'Mejoras','nav.wiki':'Wiki / Tutorial','nav.reports':'Reportes','nav.search':'Búsqueda','nav.system':'Sistema','nav.users':'Usuarios','nav.config':'Configuración','nav.logout':'Salir',
  ...
  en:{
    'nav.main':'Main','nav.dashboard':'Dashboard','nav.panoramic':'Panoramic View','nav.clients':'Clients','nav.implementations':'Implementations','nav.active':'Active','nav.canceled':'Canceled','nav.alerts':'Alerts','nav.cs':'CS Dashboard','nav.opa':'OPA / Hola Suite','nav.projects':'Projects','nav.boards':'Boards','nav.kanban':'Kanban','nav.channels':'Channels','nav.analysis':'Analysis','nav.consultants':'Consultants','nav.bi':'BI Dashboard','nav.sales':'Sales & Leads','nav.goals':'Goals','nav.tools':'Tools','nav.improvements':'Improvements','nav.wiki':'Wiki / Tutorial','nav.reports':'Reports','nav.search':'Search','nav.system':'System','nav.users':'Users','nav.config':'Settings','nav.logout':'Logout',
...
function showSection(id){
  ...
  // Lazy renders
  const lazy={bi:renderBI,ventas:renderVentas,metas:renderMetas,consultores:renderConsultores,panoramic:()=>renderPanoramic(),alertas:renderAlertas,kanban:renderKanban,tableros:renderTableros,canales:renderCanales,cs:renderCS,opa:renderOPA,wiki:()=>showWikiPage(APP.wikiCurrentPage),usuarios:renderUsuarios,mejoras:renderMejoras};
  if(lazy[id])lazy[id]();
  
  // Mostrar botón de "Nuevo Kanban" si estamos en la sección de Kanban
  const kanbanBtn = document.getElementById('newKanbanBtn');
  if(kanbanBtn) kanbanBtn.style.display = (id==='kanban'?'flex':'none');
}
...
async function syncClickUp(){
  ...
  try{
    ...
    if(tasks&&tasks.length>0){
      ...
      populateFilterSelects(); // NUEVO: Actualizar selects de filtros
...
function applyGlobalFilters() {
  const plan = document.getElementById('globalFilterPlan')?.value;
  const canal = document.getElementById('globalFilterCanal')?.value;
  const cons = document.getElementById('globalFilterCons')?.value;
  
  let data = APP.data || [];
  
  if (plan) data = data.filter(c => c.plan === plan);
  if (canal) data = data.filter(c => c[canal]);
  if (cons) data = data.filter(c => 
    c.rKickoff === cons || c.rVer === cons || c.rCap === cons || c.rGoLive === cons || c.rAct === cons || 
    (c.consultoresAsignados && c.consultoresAsignados.includes(cons))
  );
...
function populateFilterSelects() {
  const planSel = document.getElementById('globalFilterPlan');
  const consSel = document.getElementById('globalFilterCons');
  if (!planSel || !consSel) return;
  
  const data = APP.data || [];
  const planes = [...new Set(data.map(c => c.plan).filter(Boolean))].sort();
  // Extraer consultores de todas las etapas posibles
  const consultores = [...new Set(data.flatMap(c => [c.rKickoff, c.rVer, c.rCap, c.rGoLive, c.rAct]).filter(Boolean))].sort();
  
  const currentPlan = planSel.value;
  const currentCons = consSel.value;
  
  planSel.innerHTML = '<option value="">Plan: Todos</option>' + planes.map(p => `<option value="${p}">${p}</option>`).join('');
  consSel.innerHTML = '<option value="">Cons.: Todos</option>' + consultores.map(c => `<option value="${c}">${c}</option>`).join('');
  
  planSel.value = currentPlan;
  consSel.value = currentCons;
}
  ...
  if(cached){
    try{
      ...
      if(data&&data.length>0){
        ...
        populateFilterSelects(); // NUEVO: Poblar filtros al cargar cache
        APP.filteredByDate=applyDateFilter(APP.data);
        renderAll(APP.filteredByDate);
...
function renderPanoramic(){
  ...
  grid.innerHTML=data.map(c=>{
    ...
    return`<div class="pano-card ${cls}" onclick="showClient('${c.id}')">
      <div class="pano-header" style="display:flex; justify-content:space-between; align-items:start; margin-bottom:8px">
        <div class="pano-name" style="font-weight:700; font-size:14px">${escHtml(c.nombre)}</div>
        <div class="pano-icon" style="font-size:16px">${icon}</div>
      </div>
      <div class="pano-meta" style="display:flex; align-items:center; gap:6px; margin-bottom:4px">
        <span style="font-size:14px">${flagFor(c.pais)}</span>
        <span style="font-size:11px; opacity:0.8">${c.pais}</span>
      </div>
      <div class="pano-meta" style="font-size:11px; margin-bottom:8px; opacity:0.7">
        <i class="fa fa-user-tie" style="font-size:10px; margin-right:4px"></i>${escHtml(c.rCap||c.rKickoff||'—')}
      </div>
      <div style="display:flex; justify-content:space-between; align-items:center; margin-top:auto">
        <div class="pano-channels">${renderCanalesSmall(c)}</div>
        <div class="pano-days" style="font-size:10px; font-weight:700; background:rgba(255,255,255,0.05); padding:2px 6px; border-radius:4px">${c.dImpl}d</div>
      </div>
      ${c.alerta&&c.alerta!=='NO'?`<div style="font-size:9px;margin-top:6px;color:var(--danger);font-weight:600"><i class="fa fa-triangle-exclamation"></i> ${c.alerta}</div>`:''}
    </div>`;
  }).join('');
}
...
function renderKanban(){
  ...
  // Renderizar tabs si existen
  renderKanbanTabs();
  
  // Obtener filtros del kanban activo
  const activeKanban = getActiveKanbanFilters();
  
  // Si es un tablero de PROYECTO (Manual)
  if(activeKanban && activeKanban.type === 'project') {
    renderProjectKanban(activeKanban);
    return;
  }
  
  // Si es el tablero ESTÁNDAR o uno de FILTRO PERSONALIZADO
  let cons = activeKanban ? activeKanban.consultor : (document.getElementById('kanbanConsFilter')?.value||'');
  let pais = activeKanban ? activeKanban.pais : (document.getElementById('kanbanPaisFilter')?.value||'');
  let onlyAlert = activeKanban ? activeKanban.onlyAlert : (document.getElementById('kanbanOnlyAlert')?.checked||false);
  
  if(activeKanban) {
    cons = activeKanban.filterByConsultor ? activeKanban.consultor : '';
    pais = activeKanban.filterByPais ? activeKanban.pais : '';
    onlyAlert = activeKanban.onlyAlert;
  }
...
function renderProjectKanban(kb) {
  const board=document.getElementById('kanbanBoard');
  setTxt('kanbanCount', 'Tablero Personalizado');
  board.innerHTML = (kb.columns || []).map((col, cIdx) => {
    return `<div class="kanban-col" data-cidx="${cIdx}">
      <div class="kanban-col-header">
        <span class="kanban-col-title">${escHtml(col.title || col.name)}</span>
        <span class="kanban-col-count">${col.cards?.length || 0}</span>
      </div>
      <div class="kanban-items" data-cidx="${cIdx}" ondragover="handleProjectDragOver(event)" ondragleave="handleKanbanDragLeave(event)" ondrop="handleProjectDrop(event, ${cIdx})">
        ${(col.cards || []).map((card, cdIdx) => {
           return `<div class="kanban-card" draggable="true" ondragstart="handleProjectDragStart(event, ${cIdx}, ${cdIdx})">
             <div class="kanban-card-name">${escHtml(card.title || card.name)}</div>
             ${card.desc ? `<div style="font-size:11px;color:var(--muted);margin-top:4px">${escHtml(card.desc)}</div>` : ''}
             <div class="kanban-actions">
               <button class="kanban-mini-btn" onclick="deleteProjectCard(${cIdx}, ${cdIdx})"><i class="fa fa-trash"></i></button>
             </div>
           </div>`;
        }).join('')}
        <button class="btn-outline btn-sm" style="width:100%;margin-top:10px" onclick="addProjectCard(${cIdx})"><i class="fa fa-plus"></i> <span data-i18n="common.add">Añadir</span></button>
      </div>
    </div>`;
  }).join('') + `<div class="kanban-col" style="border:1px dashed var(--border); background:none; display:flex; align-items:center; justify-content:center; cursor:pointer" onclick="addProjectColumn()">
      <div style="text-align:center; color:var(--muted)"><i class="fa fa-plus"></i><br>Nueva Columna</div>
  </div>`;
}

function handleProjectDragStart(event, cIdx, cdIdx){
  event.dataTransfer.setData('project-card', JSON.stringify({cIdx, cdIdx}));
...
async function handleProjectDrop(event, toCIdx){
  event.preventDefault();
  event.currentTarget.classList.remove('kanban-drop-target');
  const raw=event.dataTransfer.getData('project-card');
  if(!raw) return;
  const {cIdx, cdIdx} = JSON.parse(raw);
  const kb = getActiveKanbanFilters();
  if(!kb || cIdx === toCIdx) return;
  
  const card = kb.columns[cIdx].cards.splice(cdIdx, 1)[0];
  kb.columns[toCIdx].cards.push(card);
  
  await updateKanbanOnServer(kb);
  renderKanban();
}

async function addProjectCard(cIdx) {
  ...
  if(!kb.columns[cIdx].cards) kb.columns[cIdx].cards = [];
  kb.columns[cIdx].cards.push({ id: Date.now(), title, desc });
  
  await updateKanbanOnServer(kb);
  renderKanban();
}

async function deleteProjectCard(cIdx, cdIdx) {
...
async function addProjectColumn() {
  ...
  const kb = getActiveKanbanFilters();
  if(!kb) return;
  kb.columns.push({ id: Date.now(), title, cards: [] });
  await updateKanbanOnServer(kb);
  renderKanban();
}

async function updateKanbanOnServer(kb) {
  try {
    const res = await fetch(`/api/kanbans/${kb.id}`, {
      method: 'PUT',
      headers: { 
...
// CUSTOM KANBANS
// ================================================================
let APP_CUSTOM_KANBANS = [];
let APP_ACTIVE_KANBAN = 'default';

function loadCustomKanbans(){
  try {
    const saved = localStorage.getItem('holaCustomKanbans');
    APP_CUSTOM_KANBANS = saved ? JSON.parse(saved) : [];
  } catch(e) {
    console.error('Error cargando kanbans personalizados:', e);
    APP_CUSTOM_KANBANS = [];
  }
  
  // Cargar desde servidor si hay token
  if(APP.token) {
    fetch('/api/kanbans', {
      headers: {
        'Authorization': 'Bearer ' + APP.token
    ...
    .then(data => {
      if(data.kanbans) {
        APP_CUSTOM_KANBANS = data.kanbans;
        renderKanbanTabs();
      }
    })
    .catch(e => console.warn('No se pudieron cargar kanbans del servidor:', e));
  }
  
  // Renderizar el kanban después de cargar datos
  renderKanban();
}

function saveCustomKanbans(){
  localStorage.setItem('holaCustomKanbans', JSON.stringify(APP_CUSTOM_KANBANS));
}

function renderKanbanTabs(){
  const tabsContainer = document.getElementById('kanbanTabs');
  if(!tabsContainer) return;
  let html = '<button class="tab-btn ' + (APP_ACTIVE_KANBAN==='default'?'active':'') + '" data-tab="default" onclick="switchKanbanTab(\'default\')">Estándar</button>';
  APP_CUSTOM_KANBANS.forEach(kb => {
    const closeBtn = '<span class="tab-close" onclick="event.stopPropagation();deleteCustomKanban(\'' + kb.id + '\')" style="position:absolute;right:6px;top:50%;transform:translateY(-50%);width:14px;height:14px;display:flex;align-items:center;justify-content:center;cursor:pointer;color:var(--muted);font-size:10px">×</span>';
    html += '<button class="tab-btn ' + (APP_ACTIVE_KANBAN===kb.id?'active':'') + '" data-tab="' + kb.id + '" onclick="switchKanbanTab(\'' + kb.id + '\')" style="position:relative;padding-right:20px">' + escHtml(kb.name) + closeBtn + '</button>';
  });
  tabsContainer.innerHTML = html;
}

function switchKanbanTab(tabId){
  APP_ACTIVE_KANBAN = tabId;
...
function showCreateKanbanModal(){
  try {
    const modal = document.createElement('div');
    modal.className = 'modal-overlay';
    modal.onclick = (e) => { if(e.target === modal) modal.remove(); };
    modal.innerHTML = `
      <div class="modal-content" style="width:520px; background:var(--dark2); border:1px solid var(--border); border-radius:20px; overflow:hidden">
        <div class="modal-header" style="padding:20px; border-bottom:1px solid var(--border)">
          <h2 style="font-size:18px; color:var(--primary)"><i class="fa fa-plus-circle"></i> Nuevo Tablero / Proyecto</h2>
          <button class="modal-close" onclick="this.closest('.modal-overlay').remove()">✕</button>
        </div>
        <div class="modal-body" style="padding:24px; display:flex; flex-direction:column; gap:20px">
          <div>
            <label style="display:block; font-size:11px; color:var(--muted); text-transform:uppercase; margin-bottom:8px">Nombre del Tablero</label>
            <input type="text" id="newKanbanName" placeholder="Ej: Implementación LATAM" style="width:100%; padding:12px; background:rgba(255,255,255,0.05); border:1px solid var(--border); border-radius:10px; color:white">
          </div>
          
          <div style="background:rgba(255,109,0,0.05); padding:16px; border-radius:12px; border:1px solid rgba(255,109,0,0.1)">
            <label style="display:block; font-size:11px; color:var(--primary); font-weight:700; text-transform:uppercase; margin-bottom:12px">Tipo de Tablero</label>
            <div style="display:grid; grid-template-columns:1fr 1fr; gap:12px">
              <label style="cursor:pointer">
                <input type="radio" name="kbType" value="custom" checked onchange="toggleKbType(this.value)" style="display:none">
                <div class="kb-type-opt active" id="opt-custom" style="padding:12px; border:1px solid var(--border); border-radius:10px; text-align:center; font-size:12px">
                  <i class="fa fa-bolt"></i><br>Automático<br><span style="font-size:10px; opacity:0.7">Filtros ClickUp</span>
                </div>
              </label>
              <label style="cursor:pointer">
                <input type="radio" name="kbType" value="project" onchange="toggleKbType(this.value)" style="display:none">
                <div class="kb-type-opt" id="opt-project" style="padding:12px; border:1px solid var(--border); border-radius:10px; text-align:center; font-size:12px">
                  <i class="fa fa-pen-ruler"></i><br>Manual<br><span style="font-size:10px; opacity:0.7">Proyecto Interno</span>
                </div>
              </label>
            </div>
          </div>
          ...
          <div id="filterInputs">
            <label style="display:block; font-size:11px; color:var(--muted); text-transform:uppercase; margin-bottom:8px">Filtros Automáticos</label>
            <div style="display:flex; flex-direction:column; gap:10px">
              <label style="display:flex; align-items:center; gap:10px; font-size:12px">
                <input type="checkbox" id="kbFilterByConsultor" style="accent-color:var(--primary)"> Por Consultor:
                <select id="kbConsultor" disabled style="flex:1; padding:6px; background:rgba(255,255,255,0.05); border:1px solid var(--border); border-radius:6px; color:white">
                  <option value="">Cualquiera...</option>
                </select>
              </label>
              <label style="display:flex; align-items:center; gap:10px; font-size:12px">
                <input type="checkbox" id="kbFilterByPais" style="accent-color:var(--primary)"> Por País:
                <select id="kbPais" disabled style="flex:1; padding:6px; background:rgba(255,255,255,0.05); border:1px solid var(--border); border-radius:6px; color:white">
                  <option value="">Cualquiera...</option>
                </select>
              </label>
              <label style="display:flex; align-items:center; gap:10px; font-size:12px; cursor:pointer">
                <input type="checkbox" id="kbOnlyAlert" style="accent-color:var(--danger)"> Solo con Alertas
              </label>
            </div>
          </div>
          ...
          <div style="display:flex; gap:12px; justify-content:flex-end; margin-top:10px">
            ...
            <button class="btn-primary" id="saveKanbanBtn">Crear Tablero</button>
          </div>
        </div>
      </div>
      <style>
        .kb-type-opt { color: var(--muted); transition: all 0.2s; }
        .kb-type-opt:hover { border-color: var(--primary); color: var(--primary); }
        .kb-type-opt.active { background: var(--primary); color: white; border-color: var(--primary); box-shadow: 0 4px 15px rgba(255,109,0,0.3); }
      </style>
    `;
    document.body.appendChild(modal);
    
    // Logic for toggling type
    window.toggleKbType = (type) => {
      document.querySelectorAll('.kb-type-opt').forEach(el => el.classList.remove('active'));
      document.getElementById('opt-' + type).classList.add('active');
      document.getElementById('filterInputs').style.display = (type === 'custom' ? 'block' : 'none');
    };

    document.getElementById('saveKanbanBtn').addEventListener('click', saveNewKanban);
    
    document.getElementById('kbFilterByConsultor').addEventListener('change', (e) => {
      document.getElementById('kbConsultor').disabled = !e.target.checked;
    });
    document.getElementById('kbFilterByPais').addEventListener('change', (e) => {
      document.getElementById('kbPais').disabled = !e.target.checked;
    });

    // Populate selects
    const consSelect = document.getElementById('kbConsultor');
    const paisSelect = document.getElementById('kbPais');
    const data = APP.data || [];
    [...new Set(data.map(c => c.rKickoff).filter(Boolean))].sort().forEach(c => {
      const opt = document.createElement('option'); opt.value = c; opt.textContent = c; consSelect.appendChild(opt);
    });
    [...new Set(data.map(c => c.pais).filter(Boolean))].sort().forEach(p => {
      const opt = document.createElement('option'); opt.value = p; opt.textContent = p; paisSelect.appendChild(opt);
    });

  } catch(e) { console.error(e); }
}

function saveNewKanban(){
  const name = document.getElementById('newKanbanName')?.value?.trim();
  if(!name) return toast('warning', 'Ingresa un nombre para el tablero');
  
  const type = document.querySelector('input[name="kbType"]:checked')?.value || 'custom';
  const config = type === 'custom' ? {
    filterByConsultor: document.getElementById('kbFilterByConsultor')?.checked,
    consultor: document.getElementById('kbConsultor')?.value,
    filterByPais: document.getElementById('kbFilterByPais')?.checked,
    pais: document.getElementById('kbPais')?.value,
    onlyAlert: document.getElementById('kbOnlyAlert')?.checked
  } : {};
  
  const columns = type === 'project' ? [
    { id: Date.now()+1, title: 'Por Hacer', cards: [] },
    { id: Date.now()+2, title: 'En Proceso', cards: [] },
    { id: Date.now()+3, title: 'Completado', cards: [] }
  ] : [];
  
  showLoading(true, 'Creando tablero...');
  fetch('/api/kanbans', {
    method: 'POST',
    headers: { 
      'Content-Type': 'application/json',
    ...
    body: JSON.stringify({ name, type, config, columns, permissions: 'private' })
  })
  .then(r => r.json())
  .then(data => {
    showLoading(false);
    if(data.error) throw new Error(data.error);
    APP_CUSTOM_KANBANS.push(data.kanban);
    renderTableros();
    renderKanbanTabs();
    document.querySelector('.modal-overlay')?.remove();
    toast('success', 'Tablero creado: ' + name);
    // Auto abrir
    switchKanbanTab(data.kanban.id);
    showSection('kanban');
  })
  .catch(e => {
    showLoading(false);
...
function deleteCustomKanban(id){
  if(!confirm('¿Eliminar este kanban personalizado?')) return;
  
  if(APP.token) {
    fetch(`/api/kanbans/${id}`, {
      method: 'DELETE',
    ...
    .then(data => {
       if(data.ok) {
         APP_CUSTOM_KANBANS = APP_CUSTOM_KANBANS.filter(kb => kb.id !== id);
         saveCustomKanbans();
         if(APP_ACTIVE_KANBAN === id) {
           APP_ACTIVE_KANBAN = 'default';
         }
         renderKanbanTabs();
         renderKanban();
         toast('success', 'Kanban eliminado');
       } else {
    ...
    APP_CUSTOM_KANBANS = APP_CUSTOM_KANBANS.filter(kb => kb.id !== id);
    saveCustomKanbans();
    if(APP_ACTIVE_KANBAN === id) APP_ACTIVE_KANBAN = 'default';
    renderKanbanTabs();
    renderKanban();
  }
}

function getActiveKanbanFilters(){
  if(APP_ACTIVE_KANBAN === 'default') return null;
  return APP_CUSTOM_KANBANS.find(kb => kb.id === APP_ACTIVE_KANBAN);
}

function renderTableros(){
  const container = document.getElementById('tablerosDashboard');
  if(!container) return;
  
  // Tablero Estándar (ClickUp)
  let html = `
    <div class="kpi-card purple" style="cursor:pointer" onclick="switchKanbanTab('default'); showSection('kanban')">
      <div class="kpi-icon"><i class="fa fa-table-columns"></i></div>
      <div class="kpi-value" style="font-size:18px">Tablero Estándar</div>
      <div class="kpi-label">ClickUp Implementaciones</div>
      <div class="kpi-sub neutral" style="margin-top:12px;border-top:1px solid rgba(255,255,255,0.05);padding-top:8px">Vista automática de tareas por etapa.</div>
      <div class="text-primary" style="margin-top:15px;font-size:12px;font-weight:700">ABRIR TABLERO <i class="fa fa-arrow-right"></i></div>
    </div>
  `;
  
  // Custom Kanbans
  APP_CUSTOM_KANBANS.forEach(kb => {
    const isManual = kb.type === 'project';
    const count = isManual ? (kb.columns?.reduce((acc, col) => acc + (col.cards?.length || 0), 0) || 0) : 'ClickUp';
    const colCount = kb.columns?.length || (isManual ? 0 : 7);
    
    html += `
      <div class="kpi-card ${isManual?'teal':'blue'}" style="position:relative">
         <div class="kpi-icon"><i class="fa ${isManual?'fa-folder-open':'fa-filter'}"></i></div>
         <div class="kpi-value" style="font-size:18px">${escHtml(kb.name)}</div>
         <div class="kpi-label">${isManual ? 'Tablero de Proyecto' : 'Tablero Filtrado'}</div>
         
         <div style="display:flex;gap:12px;margin-top:15px;font-size:11px;color:var(--muted)">
            <span><i class="fa fa-columns"></i> ${colCount} Columnas</span>
            <span><i class="fa fa-thumbtack"></i> ${count} Tarjetas</span>
         </div>
         
         <div style="display:flex;gap:8px;margin-top:20px">
           <button class="btn-primary btn-sm" style="flex:1" onclick="switchKanbanTab('${kb.id}'); showSection('kanban')">Abrir</button>
           <button class="btn-outline btn-sm" onclick="deleteCustomKanban('${kb.id}')" title="Eliminar"><i class="fa fa-trash"></i></button>
         </div>
         
         ${kb.config?.onlyAlert ? '<span class="badge badge-danger" style="position:absolute;top:10px;right:10px;font-size:9px">Solo Alertas</span>' : ''}
      </div>
    `;
  });
...
function renderCanales(){
  ...
  const channels=[
    {key:'wa',label:'WhatsApp',icon:'fab fa-whatsapp',color:'#25D366',gradient:'linear-gradient(135deg, #25D366, #128C7E)'},
    {key:'ig',label:'Instagram',icon:'fab fa-instagram',color:'#E4405F',gradient:'linear-gradient(135deg, #833AB4, #FD1D1D, #FCB045)'},
    {key:'tg',label:'Telegram',icon:'fab fa-telegram',color:'#0088CC',gradient:'linear-gradient(135deg, #0088CC, #229ED9)'},
    {key:'wc',label:'WebChat',icon:'fa fa-comments',color:'#FF6D00',gradient:'linear-gradient(135deg, #FF6D00, #FFAB40)'},
    {key:'pbx',label:'PBX',icon:'fa fa-phone',color:'#00B0FF',gradient:'linear-gradient(135deg, #00B0FF, #40C4FF)'},
    {key:'msg',label:'Messenger',icon:'fab fa-facebook-messenger',color:'#0078FF',gradient:'linear-gradient(135deg, #0078FF, #00C6FF)'}
  ];
  ...
  if(grid){
    grid.innerHTML=channels.map(ch=>{
      ...
      return`<div class="channel-overview-card" style="background:var(--card); border:1px solid var(--border); border-radius:20px; padding:20px; text-align:center; transition:all 0.3s ease; backdrop-filter:var(--glass);">
        <div class="channel-icon-wrapper" style="width:50px; height:50px; border-radius:14px; background:${ch.gradient}22; display:flex; align-items:center; justify-content:center; margin:0 auto 12px; border:1px solid ${ch.color}44">
          <i class="${ch.icon}" style="font-size:24px; color:${ch.color}"></i>
        </div>
        <div class="channel-name-big" style="font-weight:700; font-size:14px; margin-bottom:4px">${ch.label}</div>
        <div class="channel-count" style="font-size:28px; font-weight:800; color:${ch.color}">${cnt}</div>
        <div class="channel-sub" style="font-size:10px; color:var(--muted); margin-bottom:12px">${pct}% del total</div>
        <div class="channel-progress" style="height:4px; background:rgba(255,255,255,0.05); border-radius:2px; overflow:hidden">
          <div style="width:${pct}%; height:100%; background:${ch.gradient}"></div>
        </div>
        <div class="channel-breakdown" style="margin-top:12px; font-size:10px; display:flex; justify-content:space-around">
          <span style="color:var(--success)"><i class="fa fa-check-circle"></i> ${activ} Act.</span>
          <span style="color:var(--warning)"><i class="fa fa-gear"></i> ${impl} Impl.</span>
        </div>
      </div>`;
    }).join('');
  }
...
function renderVendedores(){
  ...
  const grid=document.getElementById('vendedoresGrid');
    grid.innerHTML=arr.map((v,i)=>{
      ...
      const progressColor=conversionPct>=70?'green':conversionPct>=50?'orange':'red';
      return`<div class="consultant-card" style="cursor:pointer" data-vendedor="${escHtml(v.n)}" onclick="showVendedorClientes(this.getAttribute('data-vendedor'))">
        <div class="c-header">
          ...
          <div class="c-info">
            <strong>${escHtml(v.n)}</strong>
            <span>${v.total} Ventas Totales</span>
          </div>
        </div>
        <div class="c-metrics">
          ...
          <div class="c-metric"><strong style="color:var(--danger)">${v.cancel}</strong><span>Cerrados</span></div>
        </div>
        
        <div style="display:flex; justify-content:space-between; font-size:11px; margin-bottom:6px">
          <span style="color:var(--muted)">Tasa de Retención</span>
          <span style="font-weight:700; color:var(--text)">${conversionPct}%</span>
        </div>
        <div class="progress-bar">
          <div class="progress-fill ${progressColor}" style="width:${conversionPct}%"></div>
        </div>
        
        <div class="c-stages" style="margin-top:15px">
          <span class="c-stage" style="background:rgba(0,200,83,0.08); color:var(--success)">
            <i class="fa fa-chart-line"></i> Performance: ${v.total > 5 ? 'Alto' : 'Normal'}
          </span>
          <span class="c-stage" style="background:rgba(0,176,255,0.08); color:var(--info)">
            <i class="fa fa-handshake"></i> Fidelidad: ${conversionPct}%
          </span>
        </div>
        
        <div style="text-align:center; font-size:11px; color:var(--primary); margin-top:20px; font-weight:700; text-transform:uppercase; letter-spacing:0.5px">
          <i class="fa fa-magnifying-glass"></i> Ver Cartera →
        </div>
      </div>`;
    }).join('');
  
  // Renderizar Gráfica de Vendedores
  const ctx=document.getElementById('chartVendedores');
  if(ctx && arr.length){
    if(APP.charts['chartVendedores']) APP.charts['chartVendedores'].destroy();
    APP.charts['chartVendedores'] = new Chart(ctx, {
      type: 'bar',
      data: {
        labels: arr.map(v => v.n),
        datasets: [
          { label: 'Activos', data: arr.map(v => v.activos), backgroundColor: 'rgba(0, 200, 83, 0.7)', borderRadius: 6 },
          { label: 'En Impl.', data: arr.map(v => v.impl), backgroundColor: 'rgba(255, 109, 0, 0.7)', borderRadius: 6 },
          { label: 'Cerrados', data: arr.map(v => v.cancel), backgroundColor: 'rgba(255, 23, 68, 0.4)', borderRadius: 6 }
        ]
      },
      options: {
        ...CD,
        plugins: {
          ...CD.plugins,
          legend: { display: true, position: 'top', labels: { color: 'var(--muted)', font: { size: 10 } } }
        },
        scales: {
          x: { stacked: true, grid: { display: false }, ticks: { color: 'var(--muted)' } },
          y: { stacked: true, grid: { color: 'var(--border-light)' }, ticks: { color: 'var(--muted)' } }
        }
      }
    });
  }
  ...
  document.getElementById('modalContent').innerHTML=`
    ...
    <div class="detail-grid">
      ...
      <div class="detail-item">
        <label>${t('modal.trainings_done','Cap. Realizadas')} <span class="info-tip" data-tip="Número de sesiones de capacitación realizadas.">i</span></label>
        <div style="display:flex;align-items:center;gap:8px">
          <span>${c.cantCap||0} sesiones · ${c.hCap||0}h</span>
          <button class="btn-micro" onclick="editClientHours('${c.id}')"><i class="fa fa-pen"></i></button>
        </div>
      </div>
    </div>
...
async function editClientHours(id){
  const client=APP.data.find(c=>c.id===id); if(!client)return;
  const current = client.hCap||0;
  const resPrompt=prompt(`${t('modal.trainings_done','Horas de capacitación')} para ${client.nombre} (Actual: ${current}h):\nUsa + o - para sumar/restar (ej: +2)`, current);
  if(resPrompt===null)return;
  
  let val;
  if(resPrompt.startsWith('+')) val = current + parseFloat(resPrompt.substring(1));
  else if(resPrompt.startsWith('-')) val = current - parseFloat(resPrompt.substring(1));
  else val = parseFloat(resPrompt);
  
  if(isNaN(val)){toast('error','Valor inválido');return;}
  if(val < 0) val = 0;
  
  showLoading(true,'Actualizando horas...');
  try {
    const fieldId=client.hCapFieldId || 'horas de capacitación'; 
    await updateClickUpField(id, fieldId, val);
    ...
    console.error(e);
    toast('warning','Guardado localmente. No se pudo sincronizar.');
    client.hCap=val;
    localStorage.setItem('holaData',JSON.stringify({data:APP.data,ts:Date.now(),meta:APP.apiMeta}));
    showClient(id);
  } finally {
...
async function updateClickUpField(taskId, fieldId, value) {
  const apiKey=(document.getElementById('cfgApiKey')?.value||CONFIG.API_KEY||'').trim();
  if(!apiKey) throw new Error('API Key no configurada');
  
  const res = await fetch(`/api/clickup/task/${encodeURIComponent(taskId)}/custom_field/${encodeURIComponent(fieldId)}`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
...
function shareLink(){
  ...
  if(!apiKey || !listId){
    toast('warning','Configura API Key y List ID primero');
    return;
  }
  
  const baseUrl=window.location.origin+window.location.pathname;
  const shareUrl=`${baseUrl}?apiKey=${encodeURIComponent(apiKey)}&listId=${encodeURIComponent(listId)}`;
  
  const html=`
    <div style="position:fixed;top:50%;left:50%;transform:translate(-50%,-50%);background:var(--card);border:1px solid var(--border);border-radius:28px;padding:32px;z-index:9999;max-width:550px;width:90%;box-shadow:var(--shadow);backdrop-filter:var(--glass)">
      <div style="font-size:20px;font-weight:700;margin-bottom:20px;display:flex;align-items:center;gap:12px">
        <div style="width:40px;height:40px;border-radius:12px;background:rgba(0,200,83,0.1);display:flex;align-items:center;justify-content:center;color:var(--success)">
          <i class="fa fa-share-nodes"></i>
        </div>
        Compartir Acceso
      </div>
      <p style="font-size:13px;color:var(--muted);margin-bottom:16px">Cualquier persona con este enlace podrá acceder al dashboard con los mismos datos configurados.</p>
      
      <div style="background:rgba(255,109,0,0.05);border:1px solid var(--border);border-radius:16px;padding:16px;margin-bottom:20px;font-family:monospace;font-size:12px;word-break:break-all;color:var(--primary);position:relative">
        ${shareUrl}
        <button onclick="copyToClipboard('${shareUrl}'); toast('success','URL copiada')" style="position:absolute;right:8px;top:8px;background:var(--primary);color:white;border:none;border-radius:8px;padding:6px 12px;cursor:pointer;font-size:11px">
          <i class="fa fa-copy"></i>
        </button>
      </div>
      
      <div style="display:flex;gap:12px">
        <button class="btn-primary" onclick="copyToClipboard('${shareUrl}');toast('success','Copiado')" style="flex:1;padding:14px;border-radius:12px">
          <i class="fa fa-link"></i> Copiar Enlace
        </button>
        <button class="btn-secondary" onclick="closeModal('shareModal')" style="padding:14px;border-radius:12px">
          Cerrar
        </button>
      </div>
      
      <div style="margin-top:24px;padding-top:20px;border-top:1px solid var(--border-light);display:flex;gap:12px;align-items:start">
        <i class="fa fa-network-wired" style="color:var(--info);margin-top:2px"></i>
        <div style="font-size:11px;color:var(--muted);line-height:1.5">
          <strong>Acceso Local:</strong> Asegúrate de que el firewall de tu PC permita conexiones en el puerto de este servidor si estás compartiendo en red local.
        </div>
      </div>
    </div>
  `;
  ...
  if(!modal){
    ...
    modal.id='shareModal';
    modal.style.cssText='position:fixed;top:0;left:0;right:0;bottom:0;background:rgba(0,0,0,.5);z-index:9998;display:flex;align-items:center;justify-content:center';
    modal.onclick=(e)=>{if(e.target===modal)closeModal('shareModal');};
    document.body.appendChild(modal);
  }
...
function toggleTheme(){
  ...
  localStorage.setItem('vylex_theme',next);
  updateThemeIcon(next);
  toast('info',`Tema ${next==='light'?'claro':'oscuro'} activado`);
}
...
function initTheme(){
  const saved=localStorage.getItem('vylex_theme')||'dark';
...
window.addEventListener('load',()=>{
  initTheme();
  // Generar o cargar Session ID
  let sessionId=localStorage.getItem('vylex_sessionId');
  if(!sessionId){
    sessionId='sess_'+Date.now()+'_'+Math.random().toString(36).substr(2,9);
    localStorage.setItem('vylex_sessionId',sessionId);
  }
  document.getElementById('loginSessionId').value=sessionId;
  
  // Cargar credenciales desde localStorage o URL
  const params=new URLSearchParams(window.location.search);
  const apiKeyFromUrl=params.get('apiKey');
  const listIdFromUrl=params.get('listId');
  
  // Desde localStorage si existe
  const savedApiKey=localStorage.getItem('vylex_apiKey');
  const savedListId=localStorage.getItem('vylex_listId');
  
  if(savedApiKey) document.getElementById('loginApiKey').value=savedApiKey;
  if(savedListId) document.getElementById('loginListId').value=savedListId;
  if(apiKeyFromUrl) document.getElementById('loginApiKey').value=apiKeyFromUrl;
  if(listIdFromUrl) document.getElementById('loginListId').value=listIdFromUrl;
  
  // Si están todos los datos, auto-loguear
  if((savedApiKey && savedListId) || (apiKeyFromUrl && listIdFromUrl)){
    document.getElementById('loginErrorMsg').textContent='Conectando...';
    setTimeout(()=>doAutoLogin(), 1000);
  }
